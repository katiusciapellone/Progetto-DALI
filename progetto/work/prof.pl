...
evi(sveglia):-sleep(10),write('Ciao a tutti, sono rientrato dalle ferie'),retract(ho_salutato(no)),assert(ho_salutato(si)).

decido_se_pubblicare:-specifiche_pubblicate(no),ho_salutato(si),ho_deciso(no).

evi(decido_se_pubblicare):-random(0,2,var_D),assert(pubblico(var_D)),write(var_D),nl,retract(ho_deciso(no)).

pubblicoSpecifiche:-specifiche_pubblicate(no),ho_salutato(si),pubblico(var_D),var_D==1.

evi(pubblicoSpecifiche):-write('Il progetto deve rappresentare un sistema asincrono e non deterministico. Dovete inviarmi la vostra proposta e io la valutero; se la boccio possiamo accordarci per un ricevimento alla fine del quale avrete la traccia su cui lavorare. Per la consegna potete inviarmi un messaggio con il link al repository contenente il programma. Buon lavoro!'),nl,retract(specifiche_pubblicate(no)),assert(specifiche_pubblicate(si)),broadcast.

broadcast:-studente(var_S),a(message(var_S,send_message(pubblicazione,var_Me))),fail.

eve(sollecito):-write('Uno studente ha richiesto le specifiche. Eccole.'),nl,nl,retract(pubblico(var_D)),assert(pubblico(1)),pubblicoSpecifiche.

eve(proposta(var_S,var_T)):-a(valuta(var_S,var_T)).

a(valuta(var_S,var_T)):-deterministico(var_T),write('Traccia '),write(var_T),write(' deterministica, non va bene'),nl,nl,a(message(var_S,send_message(rifiutata,var_Me))).

a(valuta(var_S,var_T)):-sincrono(var_T),write('Traccia '),write(var_T),write(' sincrona, non va bene'),nl,nl,a(message(var_S,send_message(rifiutata,var_Me))).

a(valuta(var_S,var_T)):-write('La traccia '),write(var_T),write(' va bene'),nl,nl,a(message(var_S,send_message(accettata,var_Me))),assert(assegnato(var_T,var_S)).

eve(richiestaRicevimento(var_S)):-a(message(var_S,send_message(ricevimento(giovedi),var_Me))),assert(ricevimento_con(var_S)).

eve(update(var_F,var_T)):-u(var_F,var_T).

eve(update(var_OldF,var_OldT,var_NewF,var_NewT)):-d(var_OldF,var_OldT),u(var_NewF,var_NewT).

u(var_F,var_T):-var_F=sincrono->assert(sincrono(var_T));var_F=deterministico->assert(deterministico(var_T));var_F=asincrono->assert(asincrono(var_T));var_F=non_deterministico->assert(non_deterministico(var_T)).

d(var_F,var_T):-var_F=sincrono->retract(sincrono(var_T));var_F=deterministico->retract(deterministico(var_T));var_F=asincrono->retract(asincrono(var_T));var_F=non_deterministico->retract(non_deterministico(var_T)).

eve(consegna(var_Studente,var_Messaggio)):-progetti_consegnati(var_L),append(var_L,[(var_Studente,var_Messaggio)],var_NewL),retract(progetti_consegnati(var_L)),assert(progetti_consegnati(var_NewL)),nl,write('Ho ricevuto i progetti di: '),write(var_NewL),nl.

monitoroConsegne:-progetti_consegnati(var_L),length(var_L,var_Len),var_Len>0.

evi(monitoroConsegne):-progetti_consegnati(var_L),length(var_L,var_Len),var_Len>1->a(correggo(var_L)).

a(correggo(var_L)):-nl,write('Correggo...'),nl,gestisciLista(var_L).

gestisciLista([var_H|var_T]):-nl,write('Sto valutando '),var_H=(var_S,var_M),write(var_S),write('-'),write(var_M),nl,random(1,6,var_Voto),a(message(var_S,send_message(voto(var_Voto),var_Me))),retract(progetti_consegnati([var_H|var_T])),assert(progetti_consegnati(var_T)),sleep(0.2),random(0,2,var_N),write(var_N),nl,length(var_T,var_Len),(var_N==1,var_Len>0->gestisciLista(var_T);var_Len==0->nl,write('Ho finito di correggere'),nl;nl,write('Mi fermo un attimo')),nl.
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_431207,var_Ontology,_431211),_431201),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_431245)),a(message(var_Ag,propose(var_A,[_431245],var_AgI))),retractall(ext_agent(var_Ag,_431283,var_Ontology,_431287)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_431081,var_Ontology,_431085),_431075),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_431151,var_Ontology,_431155)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_430969,var_Ontology,_430973),_430963),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_431025,var_Ontology,_431029)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_430533),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_430567),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_430349).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_430197),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_430145),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_430021,_430023)),clause(agente(_430043,_430045,_430047,var_S),_430039),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_429815,_429817,_429819,var_S),_429811),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_429915,[]),repeat,read(_429915,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_429915).
