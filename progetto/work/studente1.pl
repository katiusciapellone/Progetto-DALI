...
evi(scelgoTraccia):-write('Ora posso scegliere il progetto'),nl,random(1,8,var_T),assert(traccia(var_T)),write('Ho scelto la traccia: '),write(var_T),nl,retract(ho_scelto(no)),assert(ho_scelto(si)).

contattoProf:-ho_progetto(no),ho_scelto(si),traccia(var_T),ho_ricevimento(no).

evi(contattoProf):-nl,write('Invio proposta al professore'),nl,traccia(var_T),a(message(prof,send_message(proposta(studente1,var_T),var_Me))).

eve(accettata):-nl,write('Grazie, posso iniziare a lavorare'),nl,retract(ho_progetto(no)),assert(ho_progetto(si)).

eve(rifiutata):-nl,write('Traccia rifiutata, chiedo ricevimento'),nl,a(message(prof,send_message(richiestaRicevimento(studente1),var_Me))).
...
evi(consegno):-nl,write('Consegno il progetto'),nl,a(message(prof,send_message(consegna(studente1,link1),var_Me))),retract(ho_consegnato(no)),assert(ho_consegnato(si)).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_402681,var_Ontology,_402685),_402675),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_402719)),a(message(var_Ag,propose(var_A,[_402719],var_AgI))),retractall(ext_agent(var_Ag,_402757,var_Ontology,_402761)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_402555,var_Ontology,_402559),_402549),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_402625,var_Ontology,_402629)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_402443,var_Ontology,_402447),_402437),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_402499,var_Ontology,_402503)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_402007),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_402041),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_401823).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_401671),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_401619),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_401495,_401497)),clause(agente(_401517,_401519,_401521,var_S),_401513),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_401289,_401291,_401293,var_S),_401285),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_401389,[]),repeat,read(_401389,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_401389).
