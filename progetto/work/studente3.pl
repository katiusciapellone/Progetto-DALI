...
evi(scelgoTraccia):-write('Ora posso scegliere il progetto'),nl,assert(traccia(7)),write('Ho scelto la traccia: '),write(7),nl,retract(ho_scelto(no)),assert(ho_scelto(si)).

contattoProf:-ho_progetto(no),ho_scelto(si),traccia(var_T),ho_ricevimento(no),ho_inviato(no).

evi(contattoProf):-nl,write('Invio proposta al professore'),nl,traccia(var_T),a(message(prof,send_message(proposta(studente3,var_T),var_Me))),retract(ho_inviato(no)).

eve(accettata):-nl,write('Grazie, posso iniziare a lavorare'),nl,retract(ho_progetto(no)),assert(ho_progetto(si)).

eve(rifiutata):-nl,write('Secondo me la traccia andava bene...'),sleep(5),a(message(prof,send_message(update(deterministico,7,non_deterministico,7),studente3))),sleep(20),nl,write('Chiedo scusa per aver provato a falsificare la traccia.'),sleep(5),nl,write('Rinuncio a dare esame in questa sessione...').
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_382363,var_Ontology,_382367),_382357),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_382401)),a(message(var_Ag,propose(var_A,[_382401],var_AgI))),retractall(ext_agent(var_Ag,_382439,var_Ontology,_382443)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_382237,var_Ontology,_382241),_382231),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_382307,var_Ontology,_382311)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_382125,var_Ontology,_382129),_382119),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_382181,var_Ontology,_382185)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_381689),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_381723),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_381505).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_381353),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_381301),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_381177,_381179)),clause(agente(_381199,_381201,_381203,var_S),_381195),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_380971,_380973,_380975,var_S),_380967),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_381071,[]),repeat,read(_381071,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_381071).
